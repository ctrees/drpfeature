...
Intent
  Add ansible to  drpfeature-test-drpe_ setup for testing drp-provision endpoint used in drpfeature-test_ setup handoff to ansible-playbook.
Success
  RackN-Portal_ setup that uses inventory.py dynamic inventory to successfully run **ansible all -i inventory.py -m ping**
